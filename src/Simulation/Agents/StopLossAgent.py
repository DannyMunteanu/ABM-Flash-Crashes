...
class StopLossAgent(AgentParent):
    ...
    def _tryEnter(self, limitOrderBook: LimitOrderBook, timeTick: int) -> None:
        """
        ...
        if bestAsk is None:
            return
        size = min(self._tradeSize, self._maxPosition)
        if size <= 0:
            return
    ...
    def _tryExit(self, currentPrice: Decimal, limitOrderBook: LimitOrderBook, timeTick: int) -> None:
        """
        ...
        if currentPrice <= stopLevel or currentPrice >= targetLevel:
            size = min(self._quantity, self._maxPosition)
    ...
    def step(self, market: Market, limitOrderBook: LimitOrderBook, timeTick: int) -> None:
        ...
        if timeTick - self._lastTradeTick < self._cooldownTicks:
            return
        self._tryExit(currentPrice, limitOrderBook, timeTick)
