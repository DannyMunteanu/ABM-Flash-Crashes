...
class MesaAgentWrapper(mesa.Agent):
    ...
    def step(self, market, limitOrderBook, timeTick):
        self.inner.step(market, limitOrderBook, timeTick)
...
class FlashCrashModel(mesa.Model):
    ...
    def __init__(
        ...
        self.market = Market(self.lob, fundamentalVolatility=fundamental_vol)
        self.timeTick = 0
        ...
        if self.active_crash_ticks > 0:
            if self.lob.bestBid() is None:
                ...
                for _ in range(self._per_tick_sell_sweep):
                    ...
                    available = sum(self.lob.bidQuantity.values())
                    if available <= 0:
                        available = bid_d
                    if available <= 0:
                        break
    ...
    def get_order_book_snapshot(self, levels=10):
        ...
        bid_data = [{"price": float(p), "qty": self.lob.bidQuantity[p], "side": "bid"} for p in bids]
        ask_data = [{"price": float(p), "qty": self.lob.askQuantity[p], "side": "ask"} for p in asks]
